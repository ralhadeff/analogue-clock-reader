...
for ($i=0;$i<$ARGV[1];$i++){
    ...
    $l[0] = (rand()/5+0.4)*$r;
    # lengh of minutes hand, at least 20% larger than hour hand and up to the clock's radius
    $l[1] = rand()*($r-1.2*$l[0])+1.2*$l[0];
    # seconds hand - at least 10% larger than minutes
    $l[2] = rand()*($r-1.1*$l[1])+1.1*$l[1];
    # edge of hand
    $pi = 3.141592;
    ...
    # oposite edges (for protrutions)
    $oh[0] = $c[0] - $l[0] * cos(-$angle[0])/3;
    $oh[1] = $c[1] - $l[0] * sin(-$angle[0])/3;
    $om[0] = $c[0] - $l[0] * cos(-$angle[1])/3;
    $om[1] = $c[1] - $l[0] * sin(-$angle[1])/3;
    # seconds hands
    $ang = rand()*2*$pi;
    $sec[0] = $c[0] + $l[2] * cos(-$ang);
    $sec[1] = $c[1] + $l[2] * sin(-$ang);    
    ...
    if (rand()>0.3){
	$command.= "-stroke $st[1] -strokewidth 4 -draw 'line $c[0],$c[1] $h[0],$h[1]' ";
        $command.= "-stroke $st[1] -strokewidth 2 -draw 'line $c[0],$c[1] $m[0],$m[1]' ";
    } else {
        $command.= "-stroke $st[1] -strokewidth 4 -draw 'line $oh[0],$oh[1] $h[0],$h[1]' ";
        $command.= "-stroke $st[1] -strokewidth 2 -draw 'line $om[0],$om[1] $m[0],$m[1]' ";
    }
    # optional seconds hand
    if (rand()<0.7){
	$command.= "-stroke $st[1] -strokewidth 1 -draw 'line $c[0],$c[1] $sec[0],$sec[1]' ";
    }
